...
import { MAGIC_RECOVERY_KEY, MAGIC_RECOVERY_PASSWORD, SECRET_KEY } from '@tests/mocks';
...
describe(`Onboarding integration tests`, () => {
  ...
  it('should be able to login from welcome page then logout', async () => {
    ...
    await wallet.page.waitForSelector(wallet.$settingsButton);
    await wallet.clickSettingsButton();
    const signoutBtn = await wallet.page.$('text=Sign Out');
    expect(signoutBtn).toBeTruthy();
    await wallet.page.click('text=Sign Out');
    await wallet.page.waitForSelector(wallet.$signOutConfirmHasBackupCheckbox, { timeout: 60000 });
