...
export function StacksSwapDetails() {
  const { swapData, swappableAssetsBase } = useSwapContext<StacksSwapContext>();
  ...
  return (
    <SwapDetailsLayout>
      ...
      <SwapDetailLayout
        ...
        value={
          <HStack gap="space.01">
            <RouteNames router={routeQuoteDetails?.router} />
          </HStack>
        }
      />
      ...
      <SwapDetailLayout
        ...
        value={
          swapData.sponsorship?.isEligible
            ? 'Sponsored'
            : `${convertAmountToBaseUnit(
                swapData.fee ?? createMoney(new BigNumber(0), 'STX')
              ).toString()} STX`
        }
      />
