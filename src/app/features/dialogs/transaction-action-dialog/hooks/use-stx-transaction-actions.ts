...
export function useTransactionActions(actionType: 'cancel' | 'increaseFee') {
...
export function useStxTransactionActions(
  ...
  const { tx, isLoadingTx, rawTx, isLoadingRawTx } = useStxTransactionData(txid);
  useTransactionStatusNotifier(tx);
  const { stacksBroadcastTransaction, availableUnlockedBalance, validationSchema } =
    useTransactionActions(actionType);
