...
import {
  type StacksTransactionActionType,
  useStacksBroadcastTransaction,
} from '@app/features/stacks-transaction-request/hooks/use-stacks-broadcast-transaction';
import {
...
export function useStacksTransactionAction({ actionType, txid }: UseStacksTransactionActionArgs) {
  ...
  const onSubmitIncreaseFeeTx = useCallback(
    ...
    [tx, refreshAccountData, stacksBroadcastTransaction]
  );

  function onSubmit() {
    if (actionType === 'cancel') {
      return onSubmitCancelTx;
    }

    if (actionType === 'increaseFee') {
      return onSubmitIncreaseFeeTx;
    }
