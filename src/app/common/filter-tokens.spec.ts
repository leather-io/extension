...
import { filterTokens, isTokenEnabled, sortTokensBySwapability } from './filter-tokens';

describe('filterTokens', () => {
  const mockTokens = [{ id: 'token1' }, { id: 'token2' }, { id: 'token3' }];
  const mockAllTokens = [
...
describe('sortTokensBySwapability', () => {
  const mockTokens = [
    { id: 'principal1::contract' },
    { id: 'principal2::contract' },
    { id: 'principal3::contract' },
  ];
  const mockSwapAssets = [{ principal: 'principal2' }, { principal: 'principal3' }] as SwapAsset[];
  const getTokenIdentifier = (token: { id: string }) => token.id;

  test('sorts tokens based on swappability', () => {
    const result = sortTokensBySwapability({
      tokens: mockTokens,
